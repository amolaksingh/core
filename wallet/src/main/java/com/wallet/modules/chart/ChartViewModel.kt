...
import com.wallet.modules.coin.overview.ui.SelectedItem
...
open class ChartViewModel(
    ...
    val list = ArrayList<com.chartview.models.ChartPoint>()
        chartPointsWrapper.items.forEach {it->
            val data = com.chartview.models.ChartPoint(it.value.toFloat(),it.timestamp,it.volume?.toFloat())
            list.add(data)
        }
        val chartData = ChartData(list, chartPointsWrapper.isMovementChart, false, chartPointsWrapper.indicators)
        ...
        val headerView = if (!chartPointsWrapper.isMovementChart) {
            ...
            val currentValue = valueFormatter.formatValue(service.currency, lastItemValue)

            val dominanceData = latestItem?.let { dominance ->
                val earliestItem = chartItems.first()
                val diff = earliestItem.value?.let { earliestDominance ->
                    Value.Percent((dominance.value - earliestDominance))
                }
