...
class GlobalMarketRepository(
        ...
        return marketKit.globalMarketPointsSingle(currencyCode, chartInterval)
            .map { list ->
                list.map { point ->
                    ...
                    ChartPoint(value = value.toFloat().toBigDecimal(), timestamp = point.timestamp, volume = dominance?.toBigDecimal())
                }
            }
    }
    ...
    fun getTvlGlobalMarketPoints(
        ...
        return marketKit.marketInfoGlobalTvlSingle(chain, currencyCode, chartInterval)
            .map { list ->
                list.map { point ->
                      ChartPoint(point.value.toFloat().toBigDecimal(), point.timestamp,0.toBigDecimal())
                }
            }
    }
