...
class CoinOverviewChartService(
    ...
    private fun doGetItems(
        ...
        val chartPointList = ArrayList<io.horizontalsystems.marketkit.models.ChartPoint>()
        for (i in items.indices) {
            val chartPoint = io.horizontalsystems.marketkit.models.ChartPoint(
                items[i].value.toBigDecimal(),
                items[i].timestamp,
                items[i].volume?.toBigDecimal()
            )
            chartPointList.add(chartPoint)
        }

        return ChartPointsWrapper(chartPointList, indicators = indicators)
    }
